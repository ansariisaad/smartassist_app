...
class LeadTeams extends StatefulWidget {
  final Function onFormSubmit;
  const LeadTeams({super.key, required this.onFormSubmit});

  @override
  State<LeadTeams> createState() => _LeadTeamsState();
}

class _LeadTeamsState extends State<LeadTeams> {
  ...
  Future<void> _fetchAssigneeSearchResults(String query) async {
    ...
    try {
      ...
      final apiUrl =
          'https://api.prod.smartassistapp.in/api/search/users?user=$query';
  ...
  Future<void> fetchVehicleColors(String vehicleName) async {
    ...
    final url =
        'https://api.prod.smartassistapp.in/api/users/vehicles/all?vehicle_name=$encodedName';
  ...
  Widget _consentTick({
    ...
    return Column(
      children: [
        Align(
          alignment: Alignment.centerLeft,
          child: Text('Consent', style: AppFont.dropDowmLabel(context)),
        ),
        const SizedBox(height: 10),
        Container(
          width: double.infinity,
